...
export const store = {
    ...
    dispatch(action) {
        if (action.type === ADD_POST_ACTION_TYPE) {
            const newPost = {
                id: 4,
                message: this._state.profilePage.updateNewPost,
                likes: 0
            }
            this._state.profilePage.posts.push(newPost);
            this._state.profilePage.updateNewPost = "";
            ...
            this._state.dialogsPage.updateNewMessage = "";
            this._renderApp(this._state);
        } else if (action.type === UPDATE_TEXT_NEW_POST_ACTION_TYPE) {
            this._state.profilePage.updateNewPost = action.newText;
            this._renderApp(this._state);
        } else if (action.type === UPDATE_TEXT_NEW_MESSAGE_ACTION_TYPE) {
            this._state.dialogsPage.updateNewMessage = action.newText;
            this._renderApp(this._state);
        }
    },
